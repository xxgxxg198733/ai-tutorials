...
- Common strategies include feature extraction (frozen pre-trained weights), fine-tuning (updating all weights), and progressive unfreezing (gradual layer adaptation).
- Major types include inductive transfer (different tasks), transductive transfer/domain adaptation (same task, different domains), and unsupervised transfer (no labeled target data).
- Transfer learning has revolutionized computer vision (ImageNet pre-training) and NLP (BERT, GPT) and is now standard practice across AI.
