...
- Zero-shot learning relies on instruction tuning and broad pre-training to understand and execute natural language task descriptions.
- Meta-learning provides the theoretical foundation through algorithms that learn to learn across many tasks (MAML, prototypical networks, matching networks).
- Applications span NLP (classification, extraction, QA), vision (rare category recognition), and scientific domains (drug discovery).
- Limitations include context window constraints, sensitivity to example selection, reliability concerns, and task complexity boundaries.
